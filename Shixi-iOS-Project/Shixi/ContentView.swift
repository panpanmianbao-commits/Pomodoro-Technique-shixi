...
struct ContentView: View {
    ...
    var headerView: some View {
        HStack {
            ...
            // ✅ 自习室按钮
            NavigationLink(destination: StudyRoomView().environmentObject(vm)) {
    ...
    var normalInputView: some View {
        HStack(spacing: 8) {
            TextField("h/min/s", text: $vm.timeInput)
            ...
            VStack(spacing: 10) {
                ...
                if vm.timerMode == .pomodoro && vm.isRunning {
                    HStack(spacing: 8) {
                        ...
                        // 循环进度小圆点
                        HStack(spacing: 4) {
                            ForEach(0..<vm.pomoCycles, id: \.self) { i in
                                Circle()
